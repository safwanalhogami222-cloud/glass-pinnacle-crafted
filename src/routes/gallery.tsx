          ...
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-5 [column-fill:_balance]">
            {shown.map((src, i) => (
              ...
              >
                <img
                  src={src}
                  alt={`مشروع زجاج سيكوريت رقم ${i + 1}`}
                  loading="lazy"
                  decoding="async"
          ...
          {lightbox !== null && (
            ...
            >
              ...
              <motion.div key={lightbox} initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.25 }} className="relative max-h-[88vh] max-w-6xl w-full" onClick={(e) => e.stopPropagation()}>
                <img src={shown[lightbox]} alt={`مشروع زجاج سيكوريت رقم ${lightbox + 1}`} className="mx-auto max-h-[88vh] w-auto rounded-2xl object-contain" />
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
